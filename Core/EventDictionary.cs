...
namespace Core
{
    ...
    public class CapsEventDictionary
    {
        ...
        internal void RaiseEvent(string capsEvent, LLSD.LLSD body, Simulator simulator)
        {
            ...
            // Generic parser next
            if (body.Type == LLSDType.Map)
            {
                LLSDMap map = (LLSDMap)body;
        ...
        /// <param name="simulator">Reference to the simulator that 
        /// generated this event</param>
        internal void BeginRaiseEvent(string capsEvent, LLSD.LLSD body, Simulator simulator)
        {
            bool specialHandler = false;
            Caps.EventQueueCallback callback;
            ...
            if (body.Type == LLSDType.Map && !_EventTable.ContainsKey(capsEvent))
            {
                LLSDMap map = (LLSDMap)body;
                Packet packet = Packet.BuildPacket(capsEvent, map);
                if (packet != null)
                {
                    ...
                    Client.DebugLog("Serializing " + packet.Type + " capability with generic handler");
